...
describe("FindAllUnions", () => {
	it("should correctly find all unions on an object", () => {
		...
		type res4 = FindUnions<{ a: { b: { e: 7 | 8; f: 9 | 10 } } } | { c: 11 | 12 }, { a: { b: { e: 7; f: 9 } } }>;
		type test4 = Expect<
			Equal<
				res4,
				[
	...
	it("when matched against an empty array, the input should be turned into a union of empty array and non-empty array", () => {
		...
		type res8 = FindUnions<readonly [...number[], number], readonly [...number[], number]>;
		type tes8 = Expect<Equal<res8, []>>;

		type res9 = FindUnions<readonly [number, ...number[]], readonly [number, ...number[]]>;
