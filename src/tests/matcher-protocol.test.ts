import { isMatching, match, P } from "..";
...
describe("matcher protocol", () => {
	...
	class Some<const T> {
		...
		[P.matcher](): P.unstable_Matcher<Some<T extends P.Pattern<unknown> ? P.infer<T> : T>> {
			return {
				match: (input) => {
					return {
						matched: input instanceof Some && isMatching<any>(this.value, input.value),
					};
				},
			};
		}
	...
	it("should support taking a sub pattern", () => {
		...
		match<Option<number | string>>(new Some(12)).with(new Some(P.number), (some) => {
