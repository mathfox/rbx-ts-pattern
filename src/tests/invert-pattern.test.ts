...
describe("InvertPattern", () => {
	describe("variadic tuples", () => {
		...
		it("[a, b, ...c[]]", () => {
			type pattern1 = ["Hello", 7, ...Matcher<any, GuardP<unknown, number>, "array">[]];
			type inverted1 = InvertPattern<pattern1, unknown>;
			//    ^?
			type test1 = Expect<Equal<inverted1, ["Hello", 7, ...number[]]>>;

			type pattern2 = [GuardP<unknown, unknown>, GuardP<unknown, unknown>, ...Matcher<unknown, unknown, "array">[]];
			type inverted2 = InvertPattern<pattern2, unknown>;
			//    ^?
		...
		it("[...a[], b]", () => {
			type pattern1 = [...Matcher<any, GuardP<unknown, number>, "array">[], "Hello"];
			type inverted1 = InvertPattern<pattern1, unknown>;
			//    ^?
			type test1 = Expect<Equal<inverted1, [...number[], "Hello"]>>;

			type pattern2 = [...Matcher<unknown, unknown, "array">[], GuardP<unknown, unknown>];
			type inverted2 = InvertPattern<pattern2, unknown>;
			//    ^?
			type test2 = Expect<Equal<inverted2, [...unknown[], unknown]>>;

			type pattern3 = [...Matcher<any, GuardP<unknown, number>, "array">[], GuardP<unknown, string>];
			type inverted3 = InvertPattern<pattern3, unknown>;
			//    ^?
			type test3 = Expect<Equal<inverted3, [...number[], string]>>;
		});
		it("[...a[], b, c]", () => {
			type pattern1 = [...Matcher<any, GuardP<unknown, number>, "array">[], "Hello", 7];
			type inverted1 = InvertPattern<pattern1, unknown>;
			//    ^?
			type test1 = Expect<Equal<inverted1, [...number[], "Hello", 7]>>;

			type pattern2 = [...Matcher<unknown, unknown, "array">[], GuardP<unknown, unknown>, GuardP<unknown, unknown>];
			type inverted2 = InvertPattern<pattern2, unknown>;
			//    ^?
			type test2 = Expect<Equal<inverted2, [...unknown[], unknown, unknown]>>;
		...
		it("[a, ...b[], c]", () => {
			type pattern1 = [7, ...Matcher<any, GuardP<unknown, number>, "array">[], "Hello"];
			type inverted1 = InvertPattern<pattern1, unknown>;
			//    ^?
			type test1 = Expect<Equal<inverted1, [7, ...number[], "Hello"]>>;

			type pattern2 = [GuardP<unknown, unknown>, ...Matcher<unknown, unknown, "array">[], GuardP<unknown, unknown>];
			type inverted2 = InvertPattern<pattern2, unknown>;
			//    ^?
			type test2 = Expect<Equal<inverted2, [unknown, ...unknown[], unknown]>>;
		...
		it("[a, b, ...c[], d, e]", () => {
			type pattern1 = [7, 8, ...Matcher<any, GuardP<unknown, number>, "array">[], "Hello", "Bonjour"];
			type inverted1 = InvertPattern<pattern1, undefined>;
			//    ^?
			type test1 = Expect<Equal<inverted1, [7, 8, ...number[], "Hello", "Bonjour"]>>;
			...
			//    ^?
			type test2 = Expect<Equal<inverted2, [unknown, unknown, ...unknown[], unknown, unknown]>>;
			...
			//    ^?
			type test3 = Expect<Equal<inverted3, [string, number, ...number[], boolean, symbol]>>;
		});
	});
});
...
describe("InvertPatternForExclude", () => {
	...
	it("should work with objects", () => {
		type res1 = InvertPatternForExclude<{ a: Matcher<unknown, string> }, { a: string; b: number } | [1, 2]>;
		type test1 = Expect<Equal<res1, Readonly<{ a: string }>>>;
	});

	describe("Tuples", () => {
		it("should work with tuples", () => {
			type res1 = InvertPatternForExclude<[1, 2], { a: string; b: number } | [1, 2]>;
			type test1 = Expect<Equal<res1, [1, 2]>>;
		});

		it("should return readonly tuples if the input tuple is readonly, not otherwise", () => {
			type res1 = InvertPatternForExclude<[[[1, 2]]], { a: string } | readonly [[readonly [1, 2]]]>;
	...
	describe("variadic tuples", () => {
		it("[a, ...b[]]", () => {
			...
			type pattern4 = readonly [GuardP<unknown, unknown>, ...ArrayP<unknown, unknown>[]];
		...
		it("[a, b, ...c[]]", () => {
			type pattern1 = ["Hello", 10, ...Matcher<any, GuardP<unknown, 2>, "array">[]];
		...
		it("[...a[], b, c]", () => {
			type pattern1 = [...Matcher<any, GuardP<unknown, number>, "array">[], "Hello", true];
			type input1 = { a: string; b: number } | [...number[], string, boolean];
			type inverted1 = InvertPatternForExclude<pattern1, input1>;
			type test1 = Expect<Equal<inverted1, [...number[], "Hello", true]>>;
		});
		it("[a, ...b[], c]", () => {
			type pattern1 = ["Hello", ...Matcher<any, GuardP<unknown, number>, "array">[], true];
