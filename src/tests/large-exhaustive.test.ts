import { match, P } from "..";
import { Compute, Equal, Expect, Not } from "../types/helpers";
...
describe("large exhaustive", () => {
	...
	it("large tuple", () => {
		expect(
			match<
				[LargeObject<number>, LargeObject<number>, LargeObject<number>, LargeObject<number>, LargeObject<number>] | null
			>(null)
				.with(
					// prettier-ignore
					[
	...
	it("Input with a large distributed cardinality", () => {
		...
		const res = match(input)
			.returnType<
				{ status: "idle" } | { status: "loading" } | { status: "success" } | { status: "error"; error: unknown }
			>()
			.with(
