...
describe("ExtractPreciseValue", () => {
	it("should correctly extract the matching value from the input and an inverted pattern", () => {
		...
		type test1 = Expect<Equal<res1, ["hello", { kind: "some"; value: string }]>>;
	...
	it("should work with arrays", () => {
		type res1 = ExtractPreciseValue<boolean | { type: string } | string[], string[]>;
		type test1 = Expect<Equal<res1, string[]>>;

		type res2 = ExtractPreciseValue<({ a: string } | { b?: number | boolean; c: string })[], { b: number }[]>;
	...
	describe("Optional properties", () => {
		it("should pick the input type as the upper bound, even if it is assignable to the pattern type", () => {
			...
			type cases = [
				...
				Expect<
					Equal<ExtractPreciseValue<Input, { type: "test3" }>, { type: "test3"; id?: string; otherProp?: string }>
				>,
			];
		});
	...
	describe("non-nullable patterns", () => {
		...
		it("should exclude objects if the absent", () => {
			...
			type res2 = ExtractPreciseValue<{ a: string } | { b: number }, { b: nonNullable }>;
			type test2 = Expect<Equal<res2, { b: number }>>;

			type res3 = ExtractPreciseValue<{ a: string } | { b: number } | { b: string; c: boolean }, { b: nonNullable }>;
			type test3 = Expect<Equal<res3, { b: number } | { b: string; c: boolean }>>;
		});

		it("should keep empty objects if they come from the input type", () => {
			type res1 = ExtractPreciseValue<{ a: string } | { b: {} }, { b: nonNullable }>;
		...
		it("should exclude objects even if the non-nullable key is deeply nested", () => {
			...
			type res2 = ExtractPreciseValue<
				{ nested: { a: string } } | { nested: { b: number } } | { nested: { b: string; c: boolean } },
				{ nested: { b: nonNullable } }
			>;
			type test2 = Expect<Equal<res2, { nested: { b: number } } | { nested: { b: string; c: boolean } }>>;
		});
	});
	...
	describe("variadic patterns", () => {
		it("[a, ...b[]]", () => {
			...
			type res3 = ExtractPreciseValue<[string, ...boolean[]], ["a", ...unknown[]]>;
			type t3 = Expect<Equal<res3, ["a", ...boolean[]]>>;

			type res4 = ExtractPreciseValue<(string | boolean)[], ["a", ...unknown[]]>;
			type t4 = Expect<Equal<res4, ["a", ...(string | boolean)[]]>>;
		});

		it("[a, b, ...c[]]", () => {
			type res1 = ExtractPreciseValue<unknown[], [unknown, unknown, ...unknown[]]>;
			type t1 = Expect<Equal<res1, [unknown, unknown, ...unknown[]]>>;

			type res2 = ExtractPreciseValue<unknown[], [number, boolean, ...string[]]>;
			type t2 = Expect<Equal<res2, [number, boolean, ...string[]]>>;

			type res3 = ExtractPreciseValue<[string, number, ...boolean[]], ["a", 2, ...unknown[]]>;
		...
		it("[...a[], b]", () => {
			...
			type res3 = ExtractPreciseValue<[...boolean[], string], [...unknown[], "a"]>;
		...
		it("[...a[], b, c]", () => {
			...
			type res2 = ExtractPreciseValue<unknown[], [...string[], number, boolean]>;
			type t2 = Expect<Equal<res2, [...string[], number, boolean]>>;

			type res3 = ExtractPreciseValue<[...boolean[], string, boolean], [...unknown[], "a", true]>;
		...
		it("[a, ...b[], c]", () => {
			...
			type res2 = ExtractPreciseValue<unknown[], [number, ...string[], boolean]>;
			type t2 = Expect<Equal<res2, [number, ...string[], boolean]>>;

			type res3 = ExtractPreciseValue<[string, ...boolean[], number], ["a", ...unknown[], 2]>;
