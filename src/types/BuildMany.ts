...
// -> [value, path][]
// -> DataStructure
type BuildOne<data, xs extends readonly any[]> = xs extends [[infer value, infer path], ...infer tail]
	? BuildOne<SetDeep<data, value, path>, tail>
	: data;

// SetDeep :: a -> b -> PropertyKey[] -> a
export type SetDeep<data, value, path> = path extends readonly [infer head, ...infer tail]
