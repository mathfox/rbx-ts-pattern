...
type ReduceUnion<tuple extends readonly any[], i, output = never> = tuple extends readonly [infer p, ...infer tail]
...
export type ReduceIntersectionForExclude<tuple extends readonly any[], i, output = unknown> = tuple extends readonly [
	infer p,
	...infer tail,
]
	? ReduceIntersectionForExclude<tail, i, output & InvertPatternForExcludeInternal<p, i, unknown>>
	: output;

export type ReduceUnionForExclude<tuple extends readonly any[], i, output = never> = tuple extends readonly [
	infer p,
	...infer tail,
]
	? ReduceUnionForExclude<tail, i, output | InvertPatternForExcludeInternal<p, i, never>>
	: output;
...
type InvertPatternForExcludeInternal<p, i, empty = never> = unknown extends p // and it will end up being a false positif if we distribute it. // type is a union of literal as well as a Primitive type // We need to prevent distribution because the boolean
	? i
	: [p] extends [Primitives]
		...
		: p extends Matcher<infer matchableInput, infer subpattern, infer matcherType, any, infer excluded>
			? {
					...
					array: i extends readonly (infer ii)[]
						? MaybeAddReadonly<InvertPatternForExcludeInternal<subpattern, ii, empty>[], IsReadonlyArray<i>>
						: empty;
					map: subpattern extends [infer pk, infer pv]
